...
namespace minesmart.DGM
{
    public  class DMGService
    {
        ...
        public static async Task<JObject> dmgservicesPost(string localURL, string url, string response_data, string accessKey, string weighBridgeNo)
        {
            // eravanna / generateConfirmRawanna
            string result = "";
             DataTable dtn = new DataTable();
            JObject returnJOBJECT = new JObject();
            if (url != localURL + "eravanna/generateRawanna" && url != localURL + "tp/generateConfirmTransitPass" && url != localURL + "eravanna/generateConfirmRawanna")
            {
